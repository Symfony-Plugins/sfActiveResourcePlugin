<?php
/**
 * @author John Luxford <ux@simian.ca> http://github.com/lux
 * @author Dasher <dasher@inspiredthinking.co.uk>
 *
 * Ported to symfony from John's original implementation @see http://github.com/lux/phpactiveresource
 *
 *
 *
 */
...
class sfActiveResource {
	...
	function __construct ($data = array ()) {
		...
		$config_file_path = dirname (__FILE__) . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'config'. DIRECTORY_SEPARATOR . 'config.ini.php';
	 ...
	 */
	protected function _send_and_receive ($url, $method, $data = array ()) {
		$params = '';
	 ...
	 */
	protected function _fetch ($url, $method, $params) {
    ...
    static function callbackURL($module, $action, $controller, $fixHostAddress = true) {
      ...
      $r = $controller->genUrl(
                  '@sf_activeresource_notification?objectName='.$module.'&methodName='.$action,
                  true);
