...
class sfServiceContainerDumperPhp extends sfServiceContainerDumper
{
  ...
  protected function addServiceWrappedMethods($id, $definition) {
    ...
    if (method_exists($definition, 'hasWrappedMethods') &&  $definition->hasWrappedMethods()) {
      ...
      foreach($definition->getWrappedMethods() as $methodDefinition) {

        $methodName = $methodDefinition[0];
        $arguments = $methodDefinition[1];
        
        if (is_array($arguments) && count($arguments)>0) {
          $methodArguments = array();
          foreach($arguments as $argument) {
            $methodArguments[] = sprintf("$%s",$argument);
          }
          $methodArgs = sprintf("(%s)",implode(", ", $methodArguments));
        } else {
          $methodArgs = "()";
        }
        $code .= <<<EOF

  protected function _{$id}_{$methodName}{$methodArgs}
  {
  }
